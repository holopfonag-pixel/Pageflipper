...
const LANG_NAMES = {'auto': 'Auto', 'af': 'Afrikaans', 'sq': 'Shqip', 'am': 'አማርኛ', 'ar': 'العربية', 'hy': 'Հայերեն', 'az': 'Azərbaycan', 'eu': 'Euskara', 'be': 'Беларуская', 'bn': 'বাংলা', 'bs': 'Bosanski', 'bg': 'Български', 'ca': 'Català', 'ceb': 'Cebuano', 'ny': 'Chichewa', 'zh': '中文', 'co': 'Corse', 'hr': 'Hrvatski', 'cs': 'Čeština', 'da': 'Dansk', 'nl': 'Nederlands', 'en': 'English', 'eo': 'Esperanto', 'et': 'Eesti', 'tl': 'Filipino', 'fi': 'Suomi', 'fr': 'Français', 'fy': 'Frysk', 'gl': 'Galego', 'ka': 'ქართული', 'de': 'Deutsch', 'el': 'Ελληνικά', 'gu': 'ગુજરાતી', 'ht': 'Kreyòl Ayisyen', 'ha': 'Hausa', 'haw': 'Hawaiian', 'he': 'עברית', 'hi': 'हिन्दी', 'hmn': 'Hmong', 'hu': 'Magyar', 'is': 'Íslenska', 'ig': 'Igbo', 'id': 'Bahasa Indonesia', 'ga': 'Gaeilge', 'it': 'Italiano', 'ja': '日本語', 'jw': 'Jawa', 'kn': 'ಕನ್ನಡ', 'kk': 'Қазақ', 'km': 'ខ្មែر', 'ko': '한국어', 'ku': 'Kurdî', 'ky': 'Кыргызча', 'lo': 'ລາວ', 'la': 'Latina', 'lv': 'Latviešu', 'lt': 'Lietuvių', 'lb': 'Lëtzebuergesch', 'mk': 'Македонски', 'mg': 'Malagasy', 'ms': 'Bahasa Melayu', 'ml': 'മലയാളം', 'mt': 'Malti', 'mi': 'Maori', 'mr': 'मराठी', 'mn': 'Монгол', 'my': 'မြန်မာ', 'ne': 'नेपाली', 'no': 'Norsk', 'or': 'ଓଡ଼ିଆ', 'ps': 'پښتو', 'fa': 'فارسی', 'pl': 'Polski', 'pt': 'Português', 'pa': 'ਪੰਜਾਬੀ', 'ro': 'Română', 'ru': 'Русский', 'sm': 'Samoan', 'gd': 'Gàidhlig', 'sr': 'Српски', 'st': 'Sesotho', 'sn': 'Shona', 'sd': 'سنڌي', 'si': 'සිංහල', 'sk': 'Slovenčina', 'sl': 'Slovenščina', 'so': 'Soomaali', 'es': 'Español', 'su': 'Sunda', 'sw': 'Kiswahili', 'sv': 'Svenska', 'tg': 'Тоҷикӣ', 'ta': 'தமிழ்', 'te': 'తెలుగు', 'th': 'ไทย', 'tr': 'Türkçe', 'uk': 'Українська', 'ur': 'اردو', 'ug': 'ئۇيغۇرچە', 'uz': 'Oʻzbekcha', 'vi': 'Tiếng Việt', 'cy': 'Cymraeg', 'xh': 'IsiXhosa', 'yi': 'ייִדיש', 'yo': 'Yorùbá', 'zu': 'IsiZulu'};
...
/* ============================================================
   ...
   ============================================================ */
async function applyStaticTranslations(lang){
  const keys = [];
  const elements = [];
  ...
  document.querySelectorAll('[data-i18n], [data-i18n-title]').forEach(el => {
    ...
    // If translation doesn't exist for this lang, we'll translate it
    if (!TRANSLATIONS[lang] || !TRANSLATIONS[lang][key]) {
      keys.push(t(key, 'en')); // Use English as source
      elements.push(el);
    } else {
      const val = t(key, lang);
      if (el.hasAttribute('data-i18n')) el.textContent = val;
      if (el.hasAttribute('data-i18n-title')) el.setAttribute('title', val);
    }
  });

  if (keys.length > 0 && lang !== 'en' && lang !== 'ar') {
    try {
      const translated = await translateStrings(keys, lang, 'en');
      elements.forEach((el, i) => {
        if (el.hasAttribute('data-i18n')) el.textContent = translated[i];
        if (el.hasAttribute('data-i18n-title')) el.setAttribute('title', translated[i]);
      });
    } catch (e) { 
      // Fallback: apply default translations if dynamic fails
      elements.forEach(el => {
        const key = el.getAttribute('data-i18n') || el.getAttribute('data-i18n-title');
        const val = t(key, lang);
        if (el.hasAttribute('data-i18n')) el.textContent = val;
...
async function translateStrings(strings, lang, sourceLang = 'ar'){
  if(!strings.length) return strings;
  try{
    const joined = strings.join(SEP);
    const translated = await translateText(joined, lang, sourceLang);
...
async function translateText(text, targetLang, sourceLang = 'ar'){
  const url = CONFIG.translateEndpoint + '?client=gtx&sl=' + sourceLang + '&tl=' + encodeURIComponent(targetLang) + '&dt=t&q=' + encodeURIComponent(text);
...
async function detectLanguage(){
  const saved = storageGet(CONFIG.storageKeys.lang, 'en');
...
async function applyLanguage(lang, isAuto, announce){
  ...
  setDirForLang(lang);
  await applyStaticTranslations(lang);
